...
const DashboardAlerts: React.FC<DashboardAlertsProps> = ({ products, onNavigateToInventory }) => {
    ...
    <div className="bg-white rounded-xl border border-amber-100 shadow-sm overflow-hidden animate-in slide-in-from-top-4 duration-500">
      <div className="p-1.5 border-b border-amber-50 bg-amber-50/30 flex justify-between items-center">
        <div className="flex items-center gap-1.5">
          ...
          <div>
            ...
            <p className="text-[8px] font-bold text-amber-600 uppercase tracking-widest">Estoque Crítico (&lt; 15 un)</p>
          </div>
        </div>
        <button 
      ...
      <div className="p-1 max-h-[120px] overflow-y-auto custom-scrollbar">
        <div className="space-y-1">
          {criticalProducts.map(product => (
            <div 
              key={product.id} 
              className="flex items-center justify-between p-1.5 rounded-lg bg-slate-50 border border-slate-100 group hover:border-amber-200 hover:bg-amber-50/30 transition-all"
            >
              <div className="flex items-center gap-1.5">
                <div className="w-8 h-8 rounded-lg border bg-white overflow-hidden shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-[8px] font-bold text-slate-400 uppercase">{product.category}</p>
                </div>
              </div>
      ...
      <div className="p-1 bg-slate-50/50 border-t border-slate-100">
        <div className="flex items-center justify-center gap-1.5 text-[8px] font-bold text-slate-400 uppercase tracking-widest">
          <PackageSearch size={12} /> Total de {criticalProducts.length} itens em falta
        </div>
      </div>
    </div>
  );
